...
export default {
  ...
  computed: {
    firstRealityText() {
      return `进行现实将重置所有的游戏内容以下除外：挑战纪录和统计页面中“概况”条目下的内容。 进行现实将重置前 ${formatInt(13)} 行的所有成就，但你将每隔 ${timeDisplayNoDecimals(new Decimal(30 * 60000))} 自动获得一个成就。现实之后将基于永恒点数获得现实机器，同时获得一个符文。符文的等级基于永恒点数、复制器和膨胀时间计算。你还能获得一个可用于提升游戏体验的复兴点数，并在第一次现实之后解锁大量的现实升级。`;
    },
    canSacrifice() {
    ...
    levelStats() {
      ...
      return `现实时获得一个等级为 ${formatHybridLarge(this.level, 3)} 的符文，它的等级
        ${this.level.eq(this.bestLevel) ? "与你最高等级的符文相同" : `比你最高等级的符文
        ${this.level.gt(this.bestLevel) ? "高" : "低"} ${quantifyHybridLarge("级", this.levelDifference)}。`}`;
    },
